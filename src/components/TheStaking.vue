<template>
  <div class="flex grow flex-col">
    <div class="flex items-center mb-5 tablet:mb-10 laptop-xl:mb-16">
      <div class="basis-5/6 text-2xl font-extrabold tablet:text-3xl laptop:text-4xl laptop-xl:text-5xl">Staking with
        Vesting
      </div>
    </div>
    ...
    <div class="flex flex-col items-center" v-else>
      ...
      <div class="flex justify-between w-full bg-selection-color mb-2 p-3 rounded-lg laptop:text-lg laptop-xl:text-xl">
        <div>From</div>
        <div class="font-bold">15 Feb, 17:03</div>
      </div>
      <div class="flex justify-between w-full bg-selection-color mb-2 p-3 rounded-lg laptop:text-lg laptop-xl:text-xl">
        <div>To</div>
        <div class="font-bold">22 Mar, 17:03</div>
      </div>
      <div class="flex justify-between w-full bg-selection-color mb-2 p-3 rounded-lg laptop:text-lg laptop-xl:text-xl">
        <div>Initial stake</div>
        <div class="font-bold">200 CRT</div>
      </div>
      <div class="flex justify-between w-full bg-selection-color mb-2 p-3 rounded-lg laptop:text-lg laptop-xl:text-xl">
        <div>Current stake</div>
        <div class="font-bold">124 CRT</div>
      </div>
      <div class="flex justify-between w-full bg-selection-color mb-2 p-3 rounded-lg laptop:text-lg laptop-xl:text-xl">
        <div>APY</div>
        <div class="font-bold">36500%</div>
      </div>
      <div class="flex justify-between w-full bg-selection-color mb-2 p-3 rounded-lg laptop:text-lg laptop-xl:text-xl">
        <div>Could be unstaked</div>
        <div class="font-bold">22 CRT</div>
      </div>
    </div>
    <div class="flex flex-col justify-between mt-2 mb-4 tablet:mt-10 tablet:mb-10 laptop-xl:mt-14 laptop-xl:mb-10"
      ...
      <div class="flex items-center mb-2 tablet:mb-6">
        <input type="text" v-model.number="stakeSize" class="w-full border border-gray-200 pl-2 py-3 rounded-lg focus:outline-none
...
export default {
    ...
    calcAPY() {
      ...
      if (this.isWalletConnected === false) {
        ...
        this.stakingStrategies.forEach(staking => {
          let newTvl = staking.tvl.add(this.stakeSize);
    ...
    calcRewardsPerMonth() {
      ...
      if (this.isWalletConnected === false) {
        ...
        this.stakingStrategies.forEach(staking => {
          ...
          let rpm = null;
          try {
            rpm = BNToNumstr(staking.rewardsPerDay.mul(31).mul(this.stakeSize).div(newTvl), this.token.decimals) + ' ' + this.token.symbol;
          } catch {
  ...
  methods: {
    ...
    onStakeClick() {
      this.stake({strategy: this.selectedStrategy, stakeSize: this.stakeSize});
    },
    onUnstakeClick() {
